...
import { ledgerEntriesFor, shipmentDocFrom, stepDocFrom, traceDocsFrom } from './mirror.js';
...
export interface OrchestratorOptions {
  ...
  /** Injected so tests never make a real LLM call. Defaults to routedClient(). */
    ...
    for (const stepId of claimable) {
      ...
      if (disp.action === 'skip') {
        ...
        if (outcome.kind === 'await_approval') {
          ...
          // An LLM-negotiated deal, generated by step-runner.ts's
          // runNegotiateStep — see negotiate-llm.ts for how, and its fallback
          // to a deterministic concession when no model answers.
          if (outcome.negotiation) {
            await store.putNegotiation(outcome.negotiation.negotiation);
            await store.appendMessages(outcome.negotiation.negotiation.id, outcome.negotiation.messages);
          }
...
export class Orchestrator {
  ...
  constructor(options: OrchestratorOptions) {
    ...
    // Built once per process rather than per negotiate step: routedClient()
    // just reads env vars and wraps two fetch-based clients, so there's
    // nothing expensive to reuse, but one instance keeps every tick's
    // negotiations on the same provider choice for the process's lifetime.
    this.#llmClient = options.llmClient ?? routedClient();
  }
